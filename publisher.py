...
def load_feature_order(base_dir: Path, prefer_phase2=True):
    """Load feature order with strict precedence to ensure consistency.
    
    Args:
        base_dir: Base directory containing artifacts
        prefer_phase2: If True (default), require Phase 2 artifacts for consistency
        
    Returns:
        tuple: (feature_order: List[str], kept_idx: Optional[List[int]])
        
    Raises:
        FileNotFoundError: If required feature_order.json not found
        RuntimeError: If feature_order.json is invalid
    """
    ...
    if prefer_phase2:
        # Strict mode: require Phase 2 for consistency
        if not p2.exists():
            # Auto-fallback with warning instead of hard failure
            logger.warning(
                "Phase 2 feature_order.json not found at %s. "
                "Auto-falling back to Phase 1 feature order. "
                "This may cause model inconsistencies. "
                "Run Phase 2 (model training) to generate canonical feature order.", p2
            )
            prefer_phase2 = False  # Switch to Phase 1 fallback
        else:
            try:
            ...
            logger.info("Using Phase 2 feature order: %d features", len(fo))
        
    if not prefer_phase2:  # Now handles both explicit prefer_phase2=False and auto-fallback
        # Fallback mode: use Phase 1 (for development/testing only) 
        if not p1.exists():
            raise FileNotFoundError(f"Phase 1 feature_order.json not found at {p1}")
        
        try:
            fo = json.loads(p1.read_text())
            if not isinstance(fo, list) or len(fo) == 0:
                raise RuntimeError(f"Invalid Phase 1 feature_order.json: must be non-empty list")
                
        except (json.JSONDecodeError, IOError) as e:
            raise RuntimeError(f"Cannot read Phase 1 feature_order.json: {e}")
            
        logger.warning("Using Phase 1 feature order - this may be inconsistent with trained model!")
    
    kept_idx = None
    
    # Apply kept_feature_indices if available (for feature pruning alignment)
    if keep_idx_path.exists():
        try:
            ej = json.loads(keep_idx_path.read_text())
            kept_feature_indices = ej.get("meta", {}).get("kept_feature_indices", None)
            if kept_feature_indices:
                original_count = len(fo)
                fo = [fo[i] for i in kept_feature_indices]
                # Important: return None for kept_idx now to avoid re-applying downstream
                logging.info("Applied kept_feature_indices: %d -> %d features", original_count, len(fo))
                kept_idx = None
            else:
                logging.debug("No kept_feature_indices found in evaluation.json")
        except Exception as e:
            logging.warning("Failed to apply kept_feature_indices from evaluation.json: %s", e)
